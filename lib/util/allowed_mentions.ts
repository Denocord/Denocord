...
export default function checkAllowedMentions(
  ...
  if (!mentions) return;
  if (mentions.parse?.includes(APITypes.AllowedMentionsTypes.User) && mentions.users) {
    throw makeErrorExclusive("users");
  }
  if (mentions.parse?.includes(APITypes.AllowedMentionsTypes.Role) && mentions.roles) {
